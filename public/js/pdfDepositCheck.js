// Import from PDF Deposit Button Click Event
document.getElementById('pdfDepositButton').addEventListener('click', () => {
  ...
  const fileInput = document.getElementById('pdfDepositButton')
  if (fileInput) {
    fileInput.click()
  } else {
...
document
  .getElementById('pdfDepositButton')
...
async function pdfExcuteQuery (queries) {
  try {
    const response = await fetch('http://localhost:3000/execute-pdf-query', {
      method: 'POST',
      headers: {
