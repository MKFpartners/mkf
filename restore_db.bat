...
.\psql -U %DB_USER% -h localhost -p 5432 -lqt | findstr /C:"mkf" >nul 2>&1
if errorlevel 1 (
    echo 데이터베이스 'mkf'가 존재하지 않습니다. 생성 중...
    .\psql -U %DB_USER% -h localhost -p 5432 -c "CREATE DATABASE mkf OWNER mkf;"
    if errorlevel 1 (
    ...
    echo 기존 데이터베이스를 덮어쓰는 중...
    echo 기존 데이터베이스 연결 종료 중...
    .\psql -U %DB_USER% -h localhost -p 5432 -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = 'mkf' AND pid <> pg_backend_pid();" >nul 2>&1
    echo 기존 데이터베이스 삭제 중...
    .\psql -U %DB_USER% -h localhost -p 5432 -c "DROP DATABASE IF EXISTS mkf;"
    echo 새 데이터베이스 생성 중...
    .\psql -U %DB_USER% -h localhost -p 5432 -c "CREATE DATABASE mkf OWNER mkf;"
)
echo.
