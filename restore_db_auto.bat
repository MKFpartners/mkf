...
.\psql -U postgres -h localhost -p 5432 -t -c "SELECT 1 FROM pg_roles WHERE rolname='mkf';" | findstr /C:"1" >nul 2>&1
if errorlevel 1 (
...
if errorlevel 1 (
    ...
    echo 데이터베이스 'mkf'가 이미 존재합니다. 기존 데이터베이스를 덮어쓰는 중...
    ...
    echo 기존 데이터베이스 삭제 중...
    .\psql -U postgres -h localhost -p 5432 -c "DROP DATABASE IF EXISTS mkf;" >nul 2>&1
    echo 새 데이터베이스 생성 중...
    .\psql -U postgres -h localhost -p 5432 -c "CREATE DATABASE mkf OWNER mkf;" >nul 2>&1
)
echo.
